'''
FOURIER ANALYSES
This is a demonstration of how more and more
sine waves added together can approximate any
curve. The technique uses math involving
complex numbers and calculus.

The program starts off loading some data and
computes the first n terms in the infinete
sum of sine waves. Then follows an animation
demonstrating the effect of including more and
more terms to the sum.

This technique is particularly useful when you
need to differentiate or integrate a dataset,
since it allows you to actually to so analytically!
'''
...
noise = np.random.normal(0, 1, omegasB.size)
...
# Here I compute c_reals and c_comps which are the initial values for all the rotating vectors
...
for i in ns:
    ...
    ax.plot(times, total_us, color='C0')
    ax.plot(tsC[mask_in_time], omegasB[mask_in_time], color='black')
